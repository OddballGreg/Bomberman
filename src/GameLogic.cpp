...
//	}
	...
	void GameLogic::render()
	{
		...
		else if (state == State::PLAYING)
		{
			...
			if (_maploader._explosion.size() > 0) {
				for(int i = _maploader._explosion.size() -1; i > -1; i--)
					renderer->render(_maploader._explosion[i], "explosionTexture");
				if (--explosion_time <= 0)
					_maploader._explosion.clear();
			}
...
//				}
			...
			if (bombDropped && bombDelay != 0) {
				...
				for (float k = 0; k <= _settings->BOMB_RADIUS; k++) {
					...
					temp.offset = glm::vec3(bomb.offset.x - k, bomb.offset.y, bomb.offset.z);
					_maploader._explosion.push_back(temp);
					temp.offset = glm::vec3(bomb.offset.x, bomb.offset.y, bomb.offset.z + k);
					_maploader._explosion.push_back(temp);
					temp.offset = glm::vec3(bomb.offset.x, bomb.offset.y, bomb.offset.z - k);
					_maploader._explosion.push_back(temp);
					temp.offset = glm::vec3(bomb.offset.x, bomb.offset.y, bomb.offset.z);
				}
				_maploader._explosion.push_back(temp);
